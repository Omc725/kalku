...
const PickerColumn: React.FC<PickerColumnProps> = ({ items, value, onChange, loop = false }) => {
    ...
    const render = () => {
        ...
        if (currentInt !== s.lastInt) {
            // Kaydırma sırasında her öğe geçişinde hafif titreşim
            triggerVibration(Math.max(5, Math.round(15 * 0.5))); // Kaydırma titreşimi ana şiddetin yarısı kadar daha zarif olsun
