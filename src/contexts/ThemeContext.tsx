...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const triggerVibration = (customIntensity?: number) => {
        if (vibrationEnabled && navigator.vibrate) {
            navigator.vibrate(customIntensity || vibrationIntensity);
        }
    };
    ...
    useEffect(() => {
        ...
        if (isGlassmorphismEnabled) {
            ...
            const buttonAlpha = Math.max(0.45, glassOpacity * 1.0);
            ...
            const surfaceAlpha = Math.max(0.2, glassOpacity * 0.85);
            root.style.setProperty('--color-surface', hexToRgba(newTheme.surface, surfaceAlpha));
            root.style.setProperty('--color-keypad', hexToRgba(newTheme.keypad, surfaceAlpha));
            root.style.setProperty('--color-number-key', hexToRgba(newTheme['number-key'], Math.min(0.85, surfaceAlpha + 0.2)));
            
            const overlayAlpha = Math.min(0.94, surfaceAlpha * 1.8); 
            root.style.setProperty('--color-overlay', hexToRgba(newTheme.surface, overlayAlpha));
            ...
            const pStrong = hexToRgba(newTheme.primary, 0.95);
            const sStrong = hexToRgba(newTheme.secondary, 0.95);
            const pWeak = hexToRgba(newTheme.primary, 0.65);
            const sWeak = hexToRgba(newTheme.secondary, 0.65);
            
            const gradient = `
                radial-gradient(circle at 0% 0%, ${pStrong} 0%, transparent 65%),
                radial-gradient(circle at 100% 0%, ${sWeak} 0%, transparent 60%),
                radial-gradient(circle at 100% 100%, ${sStrong} 0%, transparent 65%),
                radial-gradient(circle at 0% 100%, ${pWeak} 0%, transparent 60%),
                radial-gradient(circle at 50% 50%, ${hexToRgba(newTheme.secondary, 0.35)} 0%, transparent 50%),
    ...
    }, [themeName, themeMode, isGlassmorphismEnabled, glassBlur, glassOpacity, vibrationEnabled, vibrationIntensity]);
